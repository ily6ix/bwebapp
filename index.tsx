...
import { 
  ...
  ChevronRight, Sparkles, Check, Scissors, Heart, Send, Award, Droplets, Target, Star, ArrowRight
} from 'lucide-react';
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} className="relative bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="bg-[#1A1A1A] p-8 text-white">
          <div className="flex items-center gap-4">
             <div className="w-12 h-12 bg-[#D4AF37]/20 rounded-2xl flex items-center justify-center text-[#D4AF37]"><Sparkles className="w-6 h-6" /></div>
             <div>
        ...
        <div className="p-8 border-t border-gray-100 bg-white">
          <div className="flex gap-3">
            <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAsk()} placeholder="How can we elevate your beauty today?" className="flex-1 bg-gray-100 border-none rounded-2xl px-6 py-4 text-sm focus:ring-1 focus:ring-[#D4AF37] outline-none" />
            <button onClick={handleAsk} className="bg-[#1A1A1A] text-white p-4 rounded-2xl hover:bg-[#D4AF37] transition-all transform hover:scale-105"><Send className="w-5 h-5" /></button>
          </div>
        </div>
      </motion.div>
    </div>
      ...
      <div className="py-24 container mx-auto px-6">
        <div className="flex flex-col lg:flex-row gap-16 items-center">
          ...
          <div className="lg:w-1/2">
            <span className="text-[#D4AF37] font-bold tracking-[0.3em] text-xs block mb-4 uppercase">The Experience</span>
            ...
            <div className="flex flex-col gap-4 mb-10">
              {['Award-winning Master Stylists', 'Organic Premium Product Lines', 'Private Consultation Suites'].map((f) => (
                <div key={f} className="flex items-center gap-3 text-[#1A1A1A] font-medium"><Check className="text-[#D4AF37] w-5 h-5" /> {f}</div>
              ))}
            </div>
            <button onClick={onOpenBooking} className="group flex items-center gap-2 text-[#D4AF37] font-bold uppercase tracking-widest text-sm hover:translate-x-2 transition-transform">Join the Elite <ArrowRight className="w-4 h-4" /></button>
          </div>
        </div>
      </div>
    </section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
        {SERVICES.map((cat, idx) => (
          <motion.div 
            ...
            transition={{ duration: 0.6, delay: idx * 0.15, ease: [0.21, 0.45, 0.32, 0.9] }}
            className="bg-white p-10 rounded-3xl shadow-sm border border-gray-100 group hover:shadow-2xl transition-all"
          >
  ...
  <section id="about" className="bg-white overflow-hidden scroll-mt-20 py-24">
    <div className="container mx-auto px-6">
      <div className="flex flex-col lg:flex-row items-center gap-20">
        ...
        <div className="lg:w-1/2">
          <span className="text-[#D4AF37] font-bold tracking-[0.4em] text-xs mb-4 block uppercase">Our Story</span>
            ...
            <div>
              ...
              <div className="space-y-10">
                <div className="flex gap-6 group">
                  <div className="w-14 h-14 bg-white/5 rounded-2xl flex items-center justify-center text-[#D4AF37] border border-white/10 group-hover:bg-[#D4AF37] group-hover:text-white transition-all duration-500"><MapPin className="w-6 h-6" /></div>
                  <div><h4 className="text-lg font-bold mb-1">Our Flagship Location</h4><p className="text-gray-400 font-light text-lg">123 Avenue de Luxe, Beverly Hills<br />California, 90210</p></div>
                </div>
                <div className="flex gap-6 group">
                  <div className="w-14 h-14 bg-white/5 rounded-2xl flex items-center justify-center text-[#D4AF37] border border-white/10 group-hover:bg-[#D4AF37] group-hover:text-white transition-all duration-500"><Phone className="w-6 h-6" /></div>
                  <div><h4 className="text-lg font-bold mb-1">Concierge Line</h4><p className="text-gray-400 font-light text-lg">+1 (555) BEAUTY-00<br />hello@beautycorner.com</p></div>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-4">
                {FAQS.map((faq, i) => (
                  <div key={i} className="border-b border-gray-200">
                    ...
                    <AnimatePresence>{activeFaq === i && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden"><p className="pb-6 text-gray-500 font-light leading-relaxed">{faq.a}</p></motion.div>)}</AnimatePresence>
                  </div>
                ))}
              </div>
            </div>
            <div className="lg:w-1/2 relative">
               <div className="sticky top-24">
                  <div className="relative rounded-[2.5rem] overflow-hidden group">
                    ...
                    <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><Star className="w-10 h-10 text-[#D4AF37]" /></div>
                  </div>
               </div>
            </div>
          </div>
...
const App = () => {
  ...
  return (
    <div className="relative min-h-screen">
      <Navbar onOpenBooking={() => setIsBookingOpen(true)} onOpenConsultant={() => setIsConsultantOpen(true)} />
      <main>
      ...
      <AnimatePresence>{isConsultantOpen && <ConsultantModal isOpen={isConsultantOpen} onClose={() => setIsConsultantOpen(false)} />}</AnimatePresence>
    </div>
  );
};
